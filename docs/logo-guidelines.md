...
名刺は **91 × 55mm 横型・表裏 2 面** で構成します。実データは `docs/business-card.html` (ブラウザの印刷で実寸 PDF 出力可)。小さめの文字と広い字間・余白を基調とします。

| 面 | 要素 | 仕様 |
|---|---|---|
| 表面 (白) | シンボルマーク | 基準版・幅 8.5mm (最小 8mm 以上を維持) |
| 表面 | 社名 | 8.5pt / 太字 / 字間 .1em |
| 表面 | 英語社名・タグライン行 | 5pt / アクセントゴールド / 字間 .22em |
| 表面 | 肩書 | 6pt / 字間 .3em |
| 表面 | 氏名 (漢字) | 12.5pt / セミボールド / 字間 .16em |
| 表面 | 氏名 (ローマ字) | 6.5pt / 字間 .24em |
| 表面 | 連絡先 | 6.4pt / 行間 1.75 / 項目ラベルはアクセントゴールド |
| 裏面 (ネイビー #3F505E) | シンボルマーク | 黒背景用 (`_dark`) 版・幅 15mm |
| 裏面 | ワードマーク | 12pt / 白 / 字間 .12em |
| 裏面 | タグライン | 5.5pt / アクセントゴールド / 字間 .3em |

- 書体はゴシック系サンセリフ (ヒラギノ角ゴ / Yu Gothic / Noto Sans JP 等) を使用します
- 氏名・住所内の区切りは全角スペース文字を使わず、余白 (CSS 等) で表現します (スペース可視化フォントでの誤印字防止)
- 表面のグラデーション罫線 (ゴールド → ネイビー) はブランドグラデーションと同一の配色です
